            ...
            <div class="display f-row f-center f-grow pB-xl">
                <table class="grid f-row">
                    <td v-for="(key, index) in grid" class="grid-block" :key="index">
                        <input :class="{'locked': key.locked, 'error': key.conflicts.length > 0}" :value="key.value" @keyup.delete="userInput(index, $event)" @keypress.prevent="userInput(index, $event)" class="normal" />
                    </td>
                </table>
            </div>
        </div>
        <div class="article f-row pV-xxl pH-l">
